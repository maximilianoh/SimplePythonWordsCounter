...
def makeImage(list_words):
    marshall_mask = np.array(Image.open("marshall_mask.png"))

    wc = WordCloud(background_color="white", max_words=2000,
                   mask=marshall_mask, contour_width=3,
    ...
    plt.axis("off")
    plt.figure()
    plt.imshow(marshall_mask, cmap=plt.cm.gray, interpolation='bilinear')
    plt.axis("off")
    plt.show()
    wc.to_file(path.join(d, "marshall.png"))
